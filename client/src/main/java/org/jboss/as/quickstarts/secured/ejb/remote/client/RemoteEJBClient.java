 ...
 */
package org.jboss.as.quickstarts.secured.ejb.remote.client;
...
import org.jboss.as.quickstarts.secured.ejb.remote.stateful.RemoteCounter;
import org.jboss.as.quickstarts.secured.ejb.remote.stateless.RemoteCalculator;
...
public class RemoteEJBClient {
	...
	private static final String STR_COLON_SEPARATOR = ":";
	...
	private static String lookupString = "jboss-secured-ejb-remote-server-side";
	...
	public static void main(String[] args) throws Exception {
		...
		System.out
				.println("argument lenght is : " + args.length + STR_NEW_LINE);
	...
	private void parseInputs(String[] args) {
		...
		try {
			...
			CommandLineArgumentsParserUtils
					.printRawArguments(args,
							"List of original arguments right after they have been parsed \n");
			if (cmdLine.getArgs().length != 0) {
				System.out
						.print("unexpected Exception occurred while processing pased in arguments: ");
	...
	private void processOptions(CommandLine cmdLine) {
		this.host = CommandLineArgumentsParserUtils.processArgument(cmdLine,
				"host");
		this.port = CommandLineArgumentsParserUtils.processArgument(cmdLine,
				"port");
		this.username = CommandLineArgumentsParserUtils.processArgument(
				cmdLine, "username");
		this.password = CommandLineArgumentsParserUtils.processArgument(
				cmdLine, "password");
		this.setRemoteEJBProps = Boolean
				.parseBoolean(CommandLineArgumentsParserUtils.processArgument(
						cmdLine, "setctxpropsp"));
		this.useSSLOnRemoteEJBProps = Boolean
				.parseBoolean(CommandLineArgumentsParserUtils.processArgument(
						cmdLine, "usessl"));
		this.ks = CommandLineArgumentsParserUtils.processArgument(cmdLine,
				"keystore");
		this.ksp = CommandLineArgumentsParserUtils.processArgument(cmdLine,
				"keystorepw");
		this.kst = CommandLineArgumentsParserUtils.processArgument(cmdLine,
				"keystoretype");
		this.ts = CommandLineArgumentsParserUtils.processArgument(cmdLine,
				"truststore");
		this.tsp = CommandLineArgumentsParserUtils.processArgument(cmdLine,
				"truststorepw");
		this.tst = CommandLineArgumentsParserUtils.processArgument(cmdLine,
				"truststoretype");
		// only override the lookup string if passed in.
		String lkpStrg = CommandLineArgumentsParserUtils.processArgument(
				cmdLine, "lookupstring");
	...
	private void printProcessedOptions() {
		...
		System.out.println("SetRemotingProps: " + this.setRemoteEJBProps
				+ STR_NEW_LINE);
		System.out.println("TuseSSLOnRemoteEJBProps: "
				+ this.useSSLOnRemoteEJBProps + STR_NEW_LINE);
	...
	private void initLookupContextProps() {
		...
		EJBClientConfiguration cc = new PropertiesBasedEJBClientConfiguration(
				props);
		ContextSelector<EJBClientContext> sel = new ConfigBasedEJBClientContextSelector(
				cc);
	...
	private static RemoteCalculator lookupRemoteStatelessCalculator()
		...
		return (RemoteCalculator) context.lookup(STR_EJB_PREFFIX + lookupString
				+ STR_SLASH_SEPARATOR + calcBeanString + STR_EXCLAM_SEPARATOR
	...
	private static RemoteCounter lookupRemoteStatefulCounter()
		...
		return (RemoteCounter) context.lookup(STR_EJB_PREFFIX + lookupString
				+ STR_SLASH_SEPARATOR + ctrBeanString + STR_EXCLAM_SEPARATOR
